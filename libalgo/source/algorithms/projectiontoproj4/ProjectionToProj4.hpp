...
std::string ProjectionToProj4::ProjectionToProj4String(const Projection <T> *proj)
{
	...
	if (proj != NULL)
	{
		...
		//Get projection name
		const char * proj_name = proj->getName();
		...
		if (i_proj_names_list != proj_names_list.end())
		{
			...
			if (proj4_name.compare(not_supported) != 0)
			{
				...
				if (typeid(*(proj)) == typeid(ProjectionConic<T>))
				{
					...
					if(strcmp(proj_name, detectprojNames[p_eqdc3]) == 0 || strcmp(proj_name, detectprojNames[p_aea]) == 0)
					{
					...
					else
					{
						...
						proj4_string += " +lat_2=" + to_string(lat0);
					}
				}
				...
				else if (strcmp(proj_name, detectprojNames[p_wer]) == 0)
				{
				...
				{
					proj4_string += " +lat_0=" + to_string(lat0);
				}
				...
				if (strcmp(proj_name, detectprojNames[p_nsper]) == 0)
				{
					const T h = R * c;
