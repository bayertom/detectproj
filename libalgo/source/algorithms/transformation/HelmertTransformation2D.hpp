                ...
                TTransformationKeyHelmert2D <typename Point1::Type> &key_helmert )
{
        ...
        if ( key_helmert.J == 0 )
        {
                throw  MathZeroDevisionException <typename Point1::Type> ( "MathZeroDevisionException: can not compute Helmert 2D transformation key, ", " divider = 0.", key_helmert.J );
        }
...
Matrix <T> HelmertTransformation2D::getTransformKey(const Matrix <T> &P, const Matrix <T> &Q, const Matrix <T> &W)
{
	...
	Matrix <T> A(2 * m, 4);
	A.replace(Q(0, m - 1, 0, 0), 0, 0);
	A.replace(Q(0, m - 1, 1, 1), m, 0);

	A.replace(-1.0 * Q(0, m - 1, 1, 1), 0, 1);
	A.replace(Q(0, m - 1, 0, 0), m, 1);
	...
	//Create Y vector
	Matrix <T> Y(2 * m, 1);
	Y.replace(P(0, m - 1, 0, 0), 0, 0);
	Y.replace(P(0, m - 1, 1, 1), m, 0);
