...
// Method: Simplex method, determined shifts
...
#ifndef FAnalyzeProjVS2_H
#define FAnalyzeProjVS2_H
...
public:
	...
	FAnalyzeProjVS2(Container <Node3DCartesian <T> *> &nl_test_, Container <Point3DGeographic <T> *> &pl_reference_, typename TMeridiansList <T> ::Type &meridians_, typename TParallelsList <T> ::Type &parallels_,
	...
	void operator () (Matrix <T> &X, Matrix <T> &Y, Matrix <T> &V, Matrix <T> &W, const bool compute_analysis = true)
	{
		...
		for (unsigned int i = 0; i < m; i++)
		{
			...
			try
			{
				...
				T lat_trans = CartTransformation::latToLatTrans(pl_reference[i]->getLat(), lon_red, X(0, 1), X(0, 2));
				T lon_trans = CartTransformation::lonToLonTrans(pl_reference[i]->getLat(), lon_red, X(0, 1), X(0, 2), trans_lon_dir);
