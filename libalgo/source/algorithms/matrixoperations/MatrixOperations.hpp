...
namespace MatrixOperations
{
	...
	Matrix <T> tridiag(const Matrix <T> &M, Matrix <T> &l, Matrix <T> &r)
	{
		...
		R(i, 0) = norm(r);
		E(i, 0) = norm(l);
		V.replace(r * (1.0 / R(i, 0)), 0, i);
		W.replace(l * (1.0 / E(i, 0)), 0, i);
	...
	void sort(Matrix <T> &A, Matrix <unsigned int> &IX, const int c)
	{
		...
		for (; i1 < i2; i1++)
		{
			...
			typename TMatrix <T>::Type col2D = col.getItems();

			//Convert to 1D vector
			typename TVector <T>::Type  col1D;
			...
			//Sort 1D vector
			std::sort(ix.begin(), ix.end(), indexComp <typename TVector <T>::Type::const_iterator>(col1D.begin(), col1D.end()));
	...
	void sortrows(Matrix <T> &A, Matrix <unsigned int> &IX, const int c)
	{
		...
		for (unsigned int i = 0; i < m1; i++)
		{
			...
			AP.replace(A(ix[i], ix[i], 0, n1 - 1), i, 0);
		}
	...
	template <typename T>
	void gemFSign(Matrix <T> &A_triangle, Matrix <T> *B)
	{
	...
	void lu(Matrix <T> A, Matrix <T> &L, Matrix <T> &U, Matrix <T> &P)
	{
		...
		for (unsigned int j = 0; j < n; j++)
		{
			...
			if ((j != i_pivot) && (i_pivot < n))
			{
				...
				U.replace(U(j, j, 0, n - 1), i_pivot, 0);
		...
		for (int i = 0; i < n; i++)
		{
			...
			while (orthogonalize)
			{
				for (int j = 0; j <= i - 1; j++)
				{
					...
					Q.replace(Q(0, m - 1, i, i) - Q(0, m - 1, j, j) * S(0, 0), 0, i);
				}
	...
	void qr(const Matrix <T> &A, Matrix <T> &Q, Matrix <T> &R, Matrix <unsigned int> &P)
	{
		...
		for (unsigned int i = 0; i < n - 1; i++)
		{
			...
			if (permut_index != i)
			{
				...
				P.replace(P(0, n - 1, permut_index, permut_index), 0, i);
				P.replace(TMP, 0, permut_index);

				//Swap R matrix
				const Matrix <T> TMP2 = R(0, m - 1, i, i);
				R.replace(R(0, m - 1, permut_index, permut_index), 0, i);
			...
			col_norms.replace(col_norms(0, 0, i + 1, n - 1) - col_norm, 0, i + 1);
		}
	}
	...
	void gill(const Matrix <T> &A, Matrix <T> &R, Matrix <T> &E, bool &indefinite, const T max_error)
	{
		...
		for (unsigned int i = 0; i < m; i++)
		{
			L.replace(L(0, m - 1, i, i) * sqrt(D(i, 0)), 0, i);
		}
	...
	void gill(const Matrix <T> &A, Matrix <T> &L, Matrix <T> &d, Matrix <T> &e, Matrix <T> &v, const T max_error)
	{
		...
		for (unsigned int i = 0; i < m; i++)
		{
			...
			if (i > 0)
			{
				if (i < n - 1)
				{
					C.replace(A(i + 1, n - 1, i, i) - C(i + 1, n - 1, 0, i - 1)  * trans(L(i, i, 0, i - 1)), i + 1, i);
				}
			}

			else
			{
				C.replace(A(i + 1, n - 1, i, i), i + 1, i);
			}
	...
	Matrix <T> mlsqr(const Matrix <T> &A, const Matrix <T> &W, const Matrix <T> &L)
	{
		...
		//Set tolerance
		const T eps = std::max(m, n) * norm(A) * EPS;
		...
		*/
		...
		for (unsigned int iter = 0; (iter < max_iter) && (index != 0); iter++)
		{
			...
			for (unsigned int i = 1; i <= index; i++)
			{
				...
				V.replace(V(0, mb - 1, i - 1, i - 1) * (-S(i, 0)) + V(0, mb - 1, i, i) * D(i, 0), 0, i);
				V.replace(CO, 0, i - 1);
			}
		...
		{
			//Copy row to permutated matrix
			VP.replace(V(IX(i, 0), IX(i, 0), 0, m - 1), i, 0);
		}
	...
	Matrix <T> pinvs(const Matrix <T> &A, const T tolerance)
	{
		...
		//Compute threshold
		const T treshold = std::max(m, n) * norm(M) * EPS;
	...
	Matrix <T> pinv2(const Matrix <T> &A)
	{
		...
		for (unsigned int i = 0; i < n; i++)
		{
			...
			//Set new matrix L
			L.replace((j == 0 ? A2(i, n - 1, i, i) : A2(i, n - 1, i, i) - L(i, n - 1, 0, j - 1) * trans(L(i, i, 0, j - 1))), i, j);
			...
			if (L(i, j) > eps)
			{
				...
				if (i < n - 1)
				{
					L.replace(L(i + 1, n - 1, j, j) / L(i, j), i + 1, j);
				}
			}
	...
	Matrix <T> hous(const Matrix <T> &A, const unsigned int i)
	{
		...
		V.replace(D(1, m - i - 1, 0, 0) / (2 * p), 1, 0);
