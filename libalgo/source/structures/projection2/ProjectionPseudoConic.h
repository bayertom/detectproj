...
template <typename T>
class ProjectionPseudoConic : public Projection <T>
{
protected:
...
public:
	ProjectionPseudoConic() : Projection <T>(), lat1(45), cart_pole(MAX_LAT, 0.0), lon_dir(NormalDirection2) {}
	ProjectionPseudoConic(const T R_, const T latp_, const T lonp_, const T lat1_, const TTransformedLongitudeDirection lon_dir_, const T lon0_, const T dx_, const T dy_, const T c_, const p_coord_function <T> &pX, const p_coord_function <T> &pY,
		const std::string name_) : Projection <T>(R_, lon0_, dx_, dy_, c_, pX, pY, name_), lat1(lat1_), cart_pole(latp_, lonp_), lon_dir(lon_dir_) {}
...
public:
	...
	virtual std::string getFamily() const { return "Pseudocon"; }
	...
	virtual TInterval <T> getLat1Interval() const { return TInterval <T> {MIN_LAT, MAX_LAT}; }
	...
	virtual void setLat2(const T lat2_) { }
	virtual void setA(const T a_) {}
	virtual void setB(const T b_) {}
