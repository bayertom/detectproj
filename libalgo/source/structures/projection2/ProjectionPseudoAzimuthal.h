...
template <typename T>
class ProjectionPseudoAzimuthal : public Projection <T>
{
protected:
...
public:
	ProjectionPseudoAzimuthal() : Projection <T>(), lat1(10.0), cart_pole(MAX_LAT, 0.0), lon_dir(NormalDirection2) {}
	ProjectionPseudoAzimuthal(const T R_, const T latp_, const T lonp_, const T lat1_, const TTransformedLongitudeDirection lon_dir_, const T lon0_, const T dx_, const T dy_, const T c_, const p_coord_function <T> &pX, const p_coord_function <T> &pY,
...
public:
	...
	virtual std::string getFamily() const { return "Pseudoaz"; }
