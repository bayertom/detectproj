...
template <typename T>
class ProjectionCylindrical : virtual public Projection <T>
{
        protected:
		...
		ProjectionCylindrical() : Projection <T>(), cart_pole(MAX_LAT, 0.0), lon_dir(NormalDirection), lat0(45.0), lat1(45.0), lat2(45.0) {}
		ProjectionCylindrical(const T R_, const T lat0_, const T lat1_, const T lat2_, const T latp_, const T lonp_, const TTransformedLongtitudeDirection lon_dir_, const T lon0_, const T dx_, const T dy_, const T c_, const char * x_equat_, const char * y_equat_, const TPostfixNotationDel &x_equat_postfix_, const TPostfixNotationDel &y_equat_postfix_, 
			const char * projection_family_, const char * projection_name_) : Projection <T>(R_, lon0_, dx_, dy_, c_, x_equat_, y_equat_, x_equat_postfix_, y_equat_postfix_, projection_family_, projection_name_), lat0(lat0_), lat1(lat1_), lat2(lat2_), cart_pole(latp_, lonp_), lon_dir(lon_dir_)
		{
		}
                virtual ~ProjectionCylindrical() {}
		...
		virtual const char * getFThetaEquatTheta() const { return NULL; }
		virtual const char * getTheta0EquatTheta() const { return NULL; }
		virtual char * getFThetaEquatTheta() { return NULL; }
		virtual char * getTheta0EquatTheta() { return NULL; }            
		...
		virtual void Theta0EquatToPostfix() {};
                ...
                virtual void getShortCut ( char * shortcut ) const { strcpy ( shortcut, "Cyli" ); }
                virtual ProjectionCylindrical <T> *clone() const {return new ProjectionCylindrical <T> ( *this );}
