...
class ProjectionMiscellaneous : virtual public Projection <T>
{
	protected:
        ...
        public:
		ProjectionMiscellaneous() : Projection <T>(), cart_pole(MAX_LAT, 0.0), lon_dir(NormalDirection2) {}
		ProjectionMiscellaneous(const T R_, const T latp_, const T lonp_, const TTransformedLongtitudeDirection lon_dir_, const T lon0_, const T dx_, const T dy_, const T c_, const char * x_equat_, const char * y_equat_, const TPostfixNotationDel &x_equat_postfix_, 
			const TPostfixNotationDel &y_equat_postfix_, const char * projection_family_, const char * projection_name_) : Projection <T>(R_, lon0_, dx_, dy_, c(c_), 
			x_equat_, y_equat_, projection_family_, projection_name_), cart_pole(latp_, lonp_), lon_dir(lon_dir_)  {}
